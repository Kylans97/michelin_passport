// Events V2 Step 7 — covers EventFriendsInterestedSection and
// EventFriendsInterestedListScreen, directly paralleling
// event_friends_going_section_test.dart's own coverage of the Going
// equivalents. Both are plain presentational widgets that take an
// already-resolved `List<Friendship>` and touch Supabase nowhere, so
// these can be pumped directly.
//
// The gating logic that decides *whether* this section is shown at all —
// omit while loading, omit on error, omit when the resolved list is
// empty, omit for a past/cancelled event, and Going-before-Interested
// ordering — lives inside _EventDetailScreenState._load()/build(), which
// this project has no way to pump (Supabase-eager); that gating is
// reviewed by direct code inspection instead, matching
// event_friends_going_section_test.dart's own documented limitation.

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:michelin_passport/core/constants/app_colors.dart';
import 'package:michelin_passport/features/events/event_friends_interested_list_screen.dart';
import 'package:michelin_passport/features/events/widgets/event_friends_interested_section.dart';
...
void main() {
  group('EventFriendsInterestedSection — preview + View all threshold', () {
    testWidgets('renders the exact "FRIENDS INTERESTED" section title', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
          EventFriendsInterestedSection(eventTitle: 'Event', friends: _one),
        ),
      );
      expect(find.text('FRIENDS INTERESTED'), findsOneWidget);
    });
    ...
    testWidgets('exactly 3 friends: all three preview, no View all', (
      ...
      await tester.pumpWidget(
        _wrap(
          EventFriendsInterestedSection(eventTitle: 'Event', friends: _three),
        ),
      );
      expect(find.text('Amy Adams'), findsOneWidget);
    ...
    testWidgets('4 friends: only the first 3 preview, View all appears', (
      ...
      await tester.pumpWidget(
        _wrap(
          EventFriendsInterestedSection(eventTitle: 'Event', friends: _four),
        ),
      );
      expect(find.text('Amy Adams'), findsOneWidget);
      expect(find.text('Bo Baker'), findsOneWidget);
    ...
    testWidgets('tapping View all opens EventFriendsInterestedListScreen '
        'with the full friend list', (tester) async {
      await tester.pumpWidget(
        _wrap(
      ...
      final screen = tester.widget<EventFriendsInterestedListScreen>(
        find.byType(EventFriendsInterestedListScreen),
      );
  ...
  group('EventFriendsInterestedListScreen', () {
    testWidgets('renders every friend, not just a preview slice', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
          home: EventFriendsInterestedListScreen(
            eventTitle: 'Event',
            friends: _four,
          ),
        ),
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: MediaQuery(
            ...
            child: EventFriendsInterestedListScreen(
              eventTitle: 'Event',
              friends: _four,
            ),
          ),
