...
## 36. Timezone hardening (event-local time, canonical model)

Added by the Events V2 Timezone Hardening pass, closing a real bug found by direct audit: `Event.fromJson` called `.toLocal()` on `start_at`/`end_at`, so every Event surface silently rendered in the *viewer's device* timezone instead of the *event's own* — a Tokyo event at 19:00 JST showed as some other hour to an Amsterdam-based viewer. Full evidence trail and backfill classification: `EVENTS_V2_TIME_LOCATION_AUDIT.md` and `EVENTS_V2_TIMEZONE_HARDENING_PRE_APPLY.md`.

**Canonical model**: `events.start_at`/`end_at` stay absolute `timestamptz` instants (unchanged); `events.timezone text` is a new column storing the IANA identifier (`Europe/Amsterdam`, `Asia/Tokyo`) the event's own instants were intended in — never an offset (`UTC+2`/`CEST`), since offsets drift with daylight saving and IANA identifiers don't. Validated at the database layer by a real Postgres-tzdata-backed trigger (`validate_event_timezone`, SQLSTATE 22023 on an unrecognized identifier), not a hand-maintained regex/allowlist. No duplicate local-datetime column was added — the pair (absolute instant, IANA zone) is sufficient to derive event-local display anywhere, which is also what makes this contract portable: Flutter today, a future Flutter Web build, and a future public website/API all derive identical event-local rendering from the same two fields, with zero server-side per-platform logic.

**Dart-side**: `Event.timezone` is `String?` — nullable because pre-backfill/pre-migration rows have none. All Event-local rendering routes through one canonical layer: `lib/core/utils/event_time.dart`'s `eventLocalDateTime(instant, timezone)` (built on `package:timezone`'s `TZDateTime.from`), consumed by `lib/features/events/event_date_format.dart`'s `formatEventDateRange`/`formatEventDateTime` and directly by `lib/models/event_trip_match.dart`'s `eventMatchesTrip`. A null/empty/unrecognized `timezone` falls back to UTC — never the device's own zone, which would silently reintroduce the exact bug this closes. `Event.fromJson` no longer calls `.toLocal()` on `start_at`/`end_at`; both stay UTC-tagged absolute instants. Every instant-based comparison (`canAttendEvent`, event sorting, the future post-event "Did you make it?" trigger) is deliberately untouched and remains correct with zero code change, since `DateTime.isAfter`/`isBefore`/`compareTo` compare the underlying instant regardless of how a `DateTime` is zone-tagged — only *display* (reading `.hour`/`.day` for formatting, or for calendar-date comparisons like `eventMatchesTrip`) was ever affected by `.toLocal()`.

**A verified gotcha for any future zone added to this list**: `Europe/Amsterdam` — the zone every event in production needs — is a backward-compatibility *link* in IANA's own tzdata (aliased to `Europe/Brussels`, identical civil-time rules since WWII), which `package:timezone`'s default (smaller) database variant excludes as "deprecated/historical." This app loads the package's `_all` database variant specifically because of that (`lib/main.dart`'s `initializeTimeZones()`), confirmed by direct empirical test, not assumed from the package's own size-savings pitch.

**Location snapshot convention (formalizing §8/§14 above as an explicit rule, not just an accidental code property)**: every Event snapshots its own physical location (`venue_name`/`address`/`city`/`country_code`/`latitude`/`longitude`) directly on the `events` row at authoring/import time. A linked host or venue's own canonical record (`event_restaurants`/`event_hotels`/`event_chefs`) is relational and contextual only — "this canonical entity was involved" — and is **never** resolved at read time as the event's location. This is already how the schema behaves today (§8/§14's case 7 is the sharpest example: a Private Chef's event location and their unrelated home-city record must never be conflated), and this section exists to make it a durable rule future schema/feature work must not violate: if a linked Restaurant or Hotel later relocates, moves cities, or is delisted, every past Event's own stored location must stay exactly as it was at the time that Event happened. Nothing in this Timezone Hardening pass changes the location schema — it was already correct — this section only records the rule explicitly.
