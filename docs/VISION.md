...
**Supersedes in spirit (not in force):** `docs/Vision/PRODUCT_VISION.md`, `docs/Vision/PRODUCT_RULES.md` — written under the product's original name, Michelin Passport. Their philosophy was sound and is carried forward here; this document itself was originally written under the product's working title, Chasing Stars, before Mantelier became the definitive name. It restates and extends that philosophy under the Mantelier identity and adds what those documents didn't yet need to answer: design, editorial, community and monetisation principles for a product that is becoming a discovery platform, not only an archive.
...
Mantelier exists to help people live a more extraordinary relationship with exceptional hospitality — to discover the tables and rooms worth travelling for, and to preserve what happened when they got there.

Two verbs, in order: **discover**, then **remember**. Neither is allowed to crowd out the other. A product that only helps people find things is a listings app. A product that only helps people log things is a diary. Mantelier is the rare thing that does both, because for the kind of traveller we build for, discovery and memory are the same continuous story.

## Vision

Mantelier becomes the definitive personal companion for people who travel for food and exceptional hospitality — the one app a member opens before a trip to decide where to go, and the one they return to afterward to keep what happened. Not the biggest restaurant database in the world. The most trusted one to a small, discerning set of people who take this seriously.

In ten years, a long-time member should be able to open Mantelier and see a legible, beautiful record of a decade of remarkable meals and stays — and, in the same app, feel confidently pointed toward what's worth their time next. Both halves should feel like they were built by the same hand, for the same person, at the same standard.

---

## Design Principles

1. **Deep green is the home, not the accent.** The canvas is Mantelier, not a neutral backdrop with Mantelier moments scattered on top. Every screen should read as unmistakably ours within half a second, before a single word is read.
...
6. **Nothing should look like it shipped with the framework.** Default Material dialogs, snackbars, list tiles, and dividers are placeholders, not a finished interface. Every visible surface earns its place in the Mantelier visual language or it doesn't ship.
...
5. **Never imply coverage we don't have.** Mantelier should never suggest — through empty-state copy, through absence of caveats, through confident-sounding defaults — that its catalogue is exhaustive. Confidence about what we do cover; honesty about what we don't.
...
5. **Community features are additive, never load-bearing.** A member with zero friends and zero social activity should still have a complete, satisfying experience. Community enriches Mantelier; it is never required to unlock its value.
...
- **History is never rewritten.** Awards, ranks, and stars change over time; Mantelier keeps the record exact for the year it happened, always. A member's memory of a two-star meal doesn't retroactively become a one-star memory because the restaurant lost a star since.
...
- **Discovery content is deterministic and explainable until it's genuinely editorial.** Until Mantelier has real editorial or community signal to work with, every "featured" or "worth the journey" selection must be traceable to a clear rule (distinction, rank, recency) — never a fabricated or arbitrary ordering presented as curation.
...
## What Mantelier Should Never Become
...
- **A discount or deals platform.** Nothing about Mantelier should ever feel like a coupon app. Exclusivity is about craft and curation, never about price.
...
- **Loud.** No gamified badge-explosions, no aggressive push-notification cadence, no urgency-manufacturing ("Only 2 spots left!"). Mantelier earns attention by being worth returning to, never by demanding it.
