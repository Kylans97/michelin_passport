...
-- 11 LOCATION_READY_CITY_PENDING restaurants (see
-- supabase/data/enrichment/michelin_netherlands_completion/verified_patch/
-- NETHERLANDS_VERIFIED_PATCH_REPORT.md and
-- MANUAL_REVIEW_RESOLUTION_ADDENDUM.md).
--
-- Adds ONLY public.cities rows -- no restaurants, no award_history, no other
-- table touched. Source: proposed_cities.csv (12 rows), re-audited against a
-- live production cities snapshot immediately before this migration was
-- written (2026-08-12, 0 drift from every prior audit in that workstream).
--
-- Idempotent by construction: ON CONFLICT targets
-- (country_code, name, coalesce(region, '')), the table's real unique index
-- (cities_unique_key) -- safe to re-run/re-push without creating
-- duplicates. Follows the exact same pattern as
-- supabase/migrations/20260812100000_expand_michelin_city_coverage.sql
-- (the Belgium/France city-coverage precedent).
--
